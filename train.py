...
def calculate_gradient_penalty(discriminator, real_data, fake_data, device, lambda_value=None):
    lambda_value = lambda_value or 10
    alpha = torch.rand(len(real_data), 1, device=device)
    ...
    grad_outputs = torch.ones(interpolated_score.size(), device=device)
...
def train(seqs, table, batch_size, latent_size, num_epoch, identity_step):
    ...
    returned = get_model_and_optimizer(latent_size, 5, 64)
    ...
    for epoch in range(1, 1 + num_epoch):
        ...
        for i, data in enumerate(dataloader):
            ...
            gp = calculate_gradient_penalty(discriminator, real_seqs, fake_seqs, device)
...
def main(fasta_path, output_root, batch_size=None, epoch=None, step=None):
    ...
    latent_size = 100
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    table_path = os.path.join(os.path.dirname(__file__), "AAF.txt") # physical_chemical_6.txt
    ...
    noise = torch.randn(len(selected_seqs), latent_size, 1, 1, device=device)
    generated_seqs = generate_seqs(generator, table, noise)
