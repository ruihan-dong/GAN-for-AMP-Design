...
def get_model_and_optimizer(latent_size, encoded_num, hidden_size):
    ...
    generator = Generator(latent_size, hidden_size, encoded_num).to(device)
    generator.apply(weights_init)
    discriminator = Discriminator(encoded_num, hidden_size).to(device)
    discriminator.apply(weights_init)
    discrim_optim = optim.RMSprop(discriminator.parameters(), lr=0.0001)
    gen_optim = optim.RMSprop(generator.parameters(), lr=0.0001)
